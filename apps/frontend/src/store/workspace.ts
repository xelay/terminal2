...
export const useWorkspaceStore = create<WorkspaceState>()(
  persist(
    (set) => ({
      ...
      removeAllIndicators: () => set({ indicators: [] }),
    }),
    {
      ...
      // Дедупликация по id после восстановления из localStorage
      merge: (persisted: any, current) => {
        const persistedState = persisted as Partial<WorkspaceState>;
        if (!persistedState.indicators) return { ...current, ...persistedState };
        const seen = new Set<string>();
        const deduped = persistedState.indicators.filter((ind: Indicator) => {
          if (seen.has(ind.id)) return false;
          seen.add(ind.id);
          return true;
        });
        return { ...current, ...persistedState, indicators: deduped };
      },
    },
  ),
);
